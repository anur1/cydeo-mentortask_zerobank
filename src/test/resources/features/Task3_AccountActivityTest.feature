...
Feature: Go to account details
  ...
  @AccountActivityTest
  Scenario: Go to Personal Account Activity Page
    Given user logs in his-her own account
    When user clicks on More Services button under Online Banking
    And user clicks on "Account Activity" link_two
    Then user sees "Show Transactions" tab
    And Account dropdown menu should contain following parts
